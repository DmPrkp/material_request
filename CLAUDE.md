...
(подписывает токены), и dictionary-server с calc-server (проверяют их). Файл обязателен, в том числе на
...
(набор заменяется целиком, одной транзакцией, кэш расчёта сбрасывается) и
...
- **Фильтруем перечисление, не фильтруем разрешение ссылки.** Списки, поиск и подбор
  видимость учитывают; `GET /{material,hand-tool}-variants?ids=|codes=` — нет
  (`VariantsService.lookup`). Разделение на своё и общее существует, чтобы заведённое
  ...
  он детерминирован, не требует токена и кэшируется одним ключом на всех. Обратная
...
- `drizzle-kit push --force` при старте dev-контейнера словаря **падает каждый раз**:
  ...
  и шпателя на него с пересчётом `code`) — тоже руками, зеркально правке сидов; так же
  пиломатериалы сведены к «Брус» (30) и «Доска обрезная» (50) со сборками 300–305 / 500–511
  (длина × ширина × толщина), позиции 31–35 и 51–61 удалены; перфораторы 15/16 переименованы
  в «перфоратор SDS+» и добавлен сетевой «перфоратор SDS-max» (26); `is_shared` у `work_types`,
  `systems`, `materials`, `hand_tools`, `power_tools`, `created_by` у `power_tools` и индексы
  имён по автору среди активных (`hand_tools_name_{ru,en}_author_uq` вместо прежних `UNIQUE`,
  `materials_name_{ru,en}_author_uq`), `systems.unit_id` (NOT NULL, FK на `units`, всем нынешним
  технологиям — `m2`) — тоже руками.
