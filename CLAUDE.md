...
- **Своё и общее** (`src/common/ownership.ts`, таблица — в README): у authored-справочников
  ...
  копией с новым id. Удалить чужое — `403`. У этапов и сборок своего флага нет, права —
  ...
  `VariantModal`, `TechnologyModal`), права там — `useOwnership()` по `sub`/`role` из токена.
...
- `drizzle-kit push --force` при старте dev-контейнера словаря **падает каждый раз**:
  ...
  `systems`, `materials`, `hand_tools`, `power_tools`, `created_by` у `power_tools` и уникальность
  имён ручного инструмента по автору среди активных (`hand_tools_name_{ru,en}_author_uq` вместо
  прежних `UNIQUE`) — тоже руками.
