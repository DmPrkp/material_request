...
лежит рядом на диске и собирается compose-ом, но версионируется отдельно — правки там
не попадут в коммит. На прод он больше **не** уезжает образом `dmprkp/calc-server:latest`
(с ним прод отставал от кода: тянулся последний запушенный latest, а не то, что деплоят).
`compose.prod.yaml` собирает его из исходников, а `deploy.sh` заливает их отдельным
`rsync` рабочей копии — в `git archive HEAD` их нет. Отсюда следствие: **на прод уезжает
рабочая копия calc-server, а не закоммиченный код**, и `/calc-server/` исключён из
основного `rsync --delete`, иначе тот снёс бы его на сервере.
