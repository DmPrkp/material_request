...
describe('error-log', () => {
  ...
  it('без LOG_DIR не пишет файлов и не падает', async () => {
    ...
    expect(() => logError('POST /user/api/v1/auth/login', new Error('лопнуло'))).not.toThrow();
  ...
  it('пишет заголовок одной строкой, а стек с отступом', async () => {
    ...
    logError('POST /user/api/v1/auth/login', new Error('relation "users" does not exist'));

    const lines = readFileSync(join(dir, 'user-server.log'), 'utf8').split('\n').slice(0, -1);
    expect(lines[0]).toMatch(
      /^\d{4}-\d{2}-\d{2}T[\d:.]+Z ERROR user-server POST \/user\/api\/v1\/auth\/login relation "users" does not exist$/,
    );
  ...
  it('дописывает вторую ошибку, а не перезаписывает файл', async () => {
    ...
    const written = readFileSync(join(dir, 'user-server.log'), 'utf8');
  ...
  it('переросший файл уводит в .1 и начинает заново', async () => {
    const { logError } = await freshLogger(dir);
    const file = join(dir, 'user-server.log');
  ...
  it('второй оборот ротации затирает прежний .1, старых файлов не копит', async () => {
    ...
    expect(readdirSync(dir).sort()).toEqual(['user-server.log', 'user-server.log.1']);
  });
