...
describe('error-log', () => {
  ...
  it('без LOG_DIR не пишет файлов и не падает', async () => {
    ...
    expect(() => logError('GET /order/api/v1/zayavka', new Error('лопнуло'))).not.toThrow();
  ...
  it('пишет заголовок одной строкой, а стек с отступом', async () => {
    ...
    logError('GET /order/api/v1/zayavka/1', new Error('relation "zayavki" does not exist'));

    const lines = readFileSync(join(dir, 'order-server.log'), 'utf8').split('\n').slice(0, -1);
    expect(lines[0]).toMatch(
      /^\d{4}-\d{2}-\d{2}T[\d:.]+Z ERROR order-server GET \/order\/api\/v1\/zayavka\/1 relation "zayavki" does not exist$/,
    );
  ...
  it('дописывает вторую ошибку, а не перезаписывает файл', async () => {
    ...
    const written = readFileSync(join(dir, 'order-server.log'), 'utf8');
  ...
  it('переросший файл уводит в .1 и начинает заново', async () => {
    const { logError } = await freshLogger(dir);
    const file = join(dir, 'order-server.log');
  ...
  it('второй оборот ротации затирает прежний .1, старых файлов не копит', async () => {
    ...
    expect(readdirSync(dir).sort()).toEqual(['order-server.log', 'order-server.log.1']);
  });
